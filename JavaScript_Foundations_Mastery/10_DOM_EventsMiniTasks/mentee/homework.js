...
const mainCardSection = document.getElementById("eventCard");
...
const counterSpanDisplay = document.getElementById("counterValue");
const incrementBtnEl = document.getElementById("incrementBtn");
const inputTextDisplay = document.getElementById("previewInput");
const spanPreviewDisplay = document.getElementById("previewText");

console.log(mainCardSection);
console.log(cardTitleEl);
console.log(cardDescriptionEl);
console.log(changeTitleBtnEl);

changeTitleBtnEl.addEventListener("click", () => {
    cardTitleEl.textContent = "Wave - 8 Events in action"
    cardDescriptionEl.textContent = "When 'clicking' the button the title will change and the description paragraph too. Both inside the event listener function";
});

toggleHighlightBtnEl.addEventListener("click", () => {
    mainCardSection.classList.toggle("event-card-highlight");
});

let counterValue = 0;
incrementBtnEl.addEventListener("click", () => {
    counterValue += 1;
    counterSpanDisplay.textContent = counterValue;
});

inputTextDisplay.addEventListener("input", () => {
    const readInputValue = inputTextDisplay.value;
    if (readInputValue === "") {
        spanPreviewDisplay.textContent = "Nothing typed yet";
    } else {
        spanPreviewDisplay.textContent = readInputValue;
    }
});
